#!/usr/bin/env python3
"""
Simple diagnostic script to check why no futures trading is being done (no external dependencies)
"""
...
def check_environment_variables():
    """Check environment variables related to futures trading"""
...
def check_configuration():
    ...
    try:
        ...
        for item in config_items:
            value = CONFIG.get(item, 'NOT_SET')
            if 'api_key' in item and value != 'NOT_SET':
                # Mask API keys for security
                value = f"{value[:10]}..." if len(value) > 10 else "***"
            print(f"{item}: {value}")
...
def check_trading_platform_selection():
    """Check which trading platform is being used"""
    print("\n=== Trading Platform Selection Check ===")
    ...
    try:
        ...
        if platform == 'binance':
            ...
            if not futures_enabled:
                ...
                print("2. Ensure your Binance account has futures enabled")
                print("3. Enable 'Enable Futures' permission in your API key")
        else:
            ...
            print("3. Configure Binance API credentials")
...
def check_futures_setup_requirements():
...
def main():
    ...
    print("Futures Trading Diagnostic (Simple)")
    print("=" * 50)
    
    checks = [
        ("Environment Variables", check_environment_variables),
        ("Configuration", check_configuration),
        ("Trading Platform Selection", check_trading_platform_selection),
        ("Futures Requirements", check_futures_setup_requirements),
    ]
